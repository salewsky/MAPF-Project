...
The idea for the node combining was taken from two other papers looking for abstractions (CITE PAPERS).The approach had to be slightly altered because my goal is to only build an abstraction and not solve the instance completely. The idea is two combine several nodes together into one node. Afterwards, the solver looks for the shortest path in the combined instance. The solver then outputs all the nodes that have been visited in the combined form (SHOW IMAGES TO EXPLAIN). The combining of nodes looks like this:
\begin{verbatim}
...
\end{verbatim}
The first rule calculates the coordinates that a node has after the combining. The constants $size\_x$ and $size\_y$ say how many nodes in x- and y-direction are combined together. The second rule is used so that it is not necessary to always care about the precombining nodes. Rule 3 and 4 change the positions of robots and shelves to their new coordinates. The rest of the encoding is only changed so that the atoms $comb\_position$ instead of $position$ is used. The output atom $imp\_position$ is then easily inferred by looking at the positions visited by robots:
\begin{verbatim}
